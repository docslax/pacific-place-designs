...
let formFunctions = {
  ...
  addSelection : function() {
    if (formFunctions.validateSelections() != false) {
      ...
      var selectedSkinTone = ($('#selected-skin-tone').val() === undefined) ? '' : $('#selected-skin-tone').val();
      var selectedBodyType = ($('#selected-body-type').val() === undefined) ? '' : $('#selected-body-type').val();
      var selectedTop = ($('#selected-top').val() === undefined) ? '' : $('#selected-top').val();
      var selectedBottom = ($('#selected-bottom').val() === undefined) ? '' : $('#selected-bottom').val();
      var selectedDrink = ($('#selected-drink').val() === undefined) ? '' : $('#selected-drink').val();
      var selectedHairColor = ($('#selected-hair-color').val() === undefined) ? '' : $('#selected-hair-color').val();
      var selectedHairStyle = $('#selected-' + $('#selected-hair-color').val().toLowerCase() + '-hair-style').val();
      var selectedAccessory = ($('#selected-accessory').val() === undefined) ? '' : $('#selected-accessory').val();
      
      var buildString = 'Name: ' + selectedName;
      buildString += formFunctions.addToBuildString('Skin Tone', selectedSkinTone);
      buildString += formFunctions.addToBuildString('Body Type', selectedBodyType);
      buildString += formFunctions.addToBuildString('Top', selectedTop);
      buildString += formFunctions.addToBuildString('Bottom', selectedBottom);
      buildString += formFunctions.addToBuildString('Drink', selectedDrink);
      buildString += formFunctions.addToBuildString('Hair Color', selectedHairColor);
      buildString += formFunctions.addToBuildString('Hair Style', selectedHairStyle);
      buildString += formFunctions.addToBuildString('Accessory', selectedAccessory);
      ...
      htmlString += formFunctions.addToHTMLString('Skin Tone', selectedSkinTone);
      htmlString += formFunctions.addToHTMLString('Body Type', selectedBodyType);
      htmlString += formFunctions.addToHTMLString('Top', selectedTop);
      htmlString += formFunctions.addToHTMLString('Bottom', selectedBottom);
      htmlString += formFunctions.addToHTMLString('Drink', selectedDrink);
      htmlString += formFunctions.addToHTMLString('Hair Color', selectedHairColor);
      htmlString += formFunctions.addToHTMLString('Hair Style', selectedHairStyle);
      htmlString += formFunctions.addToHTMLString('Accessory', selectedAccessory);
  ...
  addToBuildString : function(itemName, itemValue) {
    return (itemValue != '') ? ', ' + itemName + ': ' + itemValue : '';
  },
  addToHTMLString : function(itemName, itemValue) {
    return (itemValue != '') ? '<div class="item">' + itemName + ': ' + itemValue + '</div>' : '';
  },
